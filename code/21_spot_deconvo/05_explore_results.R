...
layer_dist_barplot <- function(counts_df, out_path, ylab, x_var, fill_var, fill_lab, xlab, fill_values) {
    p <- ggplot(
        counts_df,
...
results <- colData(spe) |>
    ...
    pivot_longer(
        starts_with("c2l_"),
        values_to = "count", names_to = "cell_type"
    ) |>
...
cell_colors <- qualitative_hcl(
    length(unique(results$cell_type)),
    palette = "Dark 3"
)
...
layer_dist_barplot(
    norm_results,
    out_path = file.path(plot_dir, "pathology_barplots.pdf"),
...
layer_dist_barplot(
    norm_results,
    out_path = file.path(plot_dir, "pathology_barplots_inverted.pdf"),
...
if ((opt$subset == "gray") || (opt$subset == "white")) {
    anov <- oneway.test(
        glial_neuron ~ path_groups, norm_results,
        var.equal = TRUE
    )
    message(
        paste(
