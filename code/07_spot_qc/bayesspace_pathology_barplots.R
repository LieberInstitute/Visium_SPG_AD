...
for (i in colnames(bayes_cols)) {
    ...
    plot_df <- cluster_df %>% group_by(!!sym(i), sample_id) %>% summarize(percent_outlier = sum(pTau_outliers == "outlier") / n() * 100, count = sum(pTau_outliers == "outlier"))

    line_df <- plot_df %>% group_by(sample_id) %>% summarize(mean_line = mean(percent_outlier))
...
for (i in colnames(bayes_cols)) {
    ...
    plot_df <- cluster_df %>% group_by(!!sym(i), sample_id) %>% summarize(percent_outlier = sum(Abeta_outliers == "outlier") / n() * 100, count = sum(Abeta_outliers == "outlier"))
