...
load(file.path(dir_rdata, "pathology_model_results_raw.Rdata"), verbose = TRUE)
...
f_merge <- function(p, fdr, t) {
    ...
    stopifnot(identical(rownames(res), rownames(sce_pseudo)))
    res$gene <- rowData(sce_pseudo)$gene_name
    rownames(res) <- NULL
    return(res)
}
